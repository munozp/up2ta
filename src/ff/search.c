...

{
  ...
  while ( TRUE ) {
	...
	printf("\n                        %4d", min);
			...
			/*
			 * Hay que comprobar que tanto la heuristica que calcula el planificador ff 
			 * como la que proporciona el proceso hijo sean 0 antes de imprimir el plan.
			 */
			    if ( (first->h == 0) && (comparador_igual(first->h_double,0.0) == 1) ) {
			      break;
			    }
...

{
   ...
   */
			...
			/*
		  	 * Hay que quitarlo para que añada se permitan nodos repetidos en la lista de abiertos.
			 * Hay que tener en cuenta que se puede llegar a un nodo por distintos caminos.
  ...
  if ( h == INFINITY ) {
    return;
			...
			/*
			 * Si el padre es nulo (father == NULL) o realiza una tarea mas (h < father->h)
			 * se annade el nodo a la lista de abiertos.
			 * Si el padre es nulo significa que es el nodo inicial a partir del cual
			 * nacen los planes del planificador.
			 */

			if ((father == NULL) || (h < father->h))
			{

/*
			 * El nodo inicial tiene op=-1. Este nodo no representa ninguna accion,
			 * es simplemente un nodo inicial a partir del cual parten todos los posibles
			 * planes que se pueden realizar. Cada uno de los nodos hijos del nodo inicial
			 * seran los distintos planes que se pueden realizar empezando por diferentes
			 * acciones.
			 */
			...
			{
				/*
				 * El coste del nodo es, el coste acumulado que tenga el padre +
				 * el coste que cuesta hacer la accion que el nodo padre representa.
				 */
				if ((father!=NULL) && (father->op >= 0)){
					coste = getCoste(father->op) + father->coste;
				}

				/*
				 * Si aun quedan tareas por planificar (h > 0) se calcula la heuristica del nodo.
				 * Si ya no quedan (h = 0) hay que añadir al coste del plan, el coste de realizar
				 * la ultima tarea (h_doube = 0.0 y coste += coste_de_la_tarea).
				 */
				if (h > 0){
					h_double = getHeuristica(op);
				} else {
					h_double = 0.0;
					coste += getCoste(op);
				}
			...
			double next, this;
			next = i->next->h_double + i->next->coste;
			this = h_double + coste;
			if (comparador_mayor_igual(next,this) == 1)			//(i->next->h_double + i->next->coste) >= (h_double + coste)
			{
				if (comparador_mayor(next,this) == 1)			//(i->next->h_double + i->next->coste) > (h_double + coste)
				{
					break;
				}
				else 							//((i->next->h_double + i->next->coste) == (h_double + coste))
				{
					next = i->next->coste;
					this = coste;
					if (comparador_mayor_igual(next,this) == 1)	//(i->next->coste) >= (coste)
					{
						if (comparador_mayor(next,this) == 1)	//(i->next->coste) > (coste)
						{
							break;
						}
						else 					//(i->next->coste == coste)
						{
							if (i->next->h > h) 
							{
								break;
  ...
  new->h = h;
			/*
			 * Se asignan los valores que proporciona el proceso hijo
			 * al nuevo nodo.
			 */
if(h_double == 0)
                new->h_double=0;
...
new->h_double = 1;
  ...
  if ( new->next ) {
    new->next->prev = new;
			...
			} /*END_IF (if ((father == NULL) || (h < father->h)))*/
