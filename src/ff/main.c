...
struct tms lstart, lend;
	...
	int fdPipeAstarff, fdPipeffAstar;
...

{
	...
	/*
	 * Se modifica el main original incluyendo un fork() que clonara el proceso.
	 * El proceso hijo se encarga de ejecutar el programa ./busqueda que proporciona
	 * costes y heuristicas al proceso padre para que realice su busqueda.
	 * El proceso padre, crea las tuberias necesarias para comunicarse con el proceso hijo
	 * y continua con el comportamiento original.
	 */

	if(argc < 7)
	{
		printf("Task-PathPlanner: insuficient args.\n");
		printf("./ff -o operators.pddl -f facts.pddl .path-algorithm[c] demfile costfile [outfile]\n");
		exit(-1);
	}
	...
	if(pid == 0){
		/*Proceso hijo*/
		if(argc == 7)
			execl("/usr/share/pathplanning.sh", "/usr/share/pathplanning.sh",argv[5],argv[6],argv[7],NULL);
		else
			execl("/usr/share/pathplanning.sh", "/usr/share/pathplanning.sh",argv[5],argv[6],argv[7],argv[8],NULL);
		perror("execl(/usr/share/pathplanning.sh);: ");
		exit (-1);
	} else {
		/*Proceso padre*/
		if (pid > 0) {
			fdPipeAstarff = abrirTuberiaLectura (NOMBRE_TUBERIA_AstarFF, 460, O_RDONLY);
			fdPipeffAstar = abrirTuberiaEscritura (NOMBRE_TUBERIA_FFAstar, 460, O_WRONLY);
			inicializar(&costes);
			inicializar(&heuristicas);
...
/*
  ...
  if ( found_plan ) {
    print_plan();
			...
			/*
			 * En consecuencia de la modificacion descrita al comienzo del main, hay que
			 * esperar el fin del proceso hijo y cerrar las tuberias de intercomunicacion.
			 * Si se ha producido algun error en el proceso se detecta.
			 */
			...
			cerrarTuberia(fdPipeAstarff);
			cerrarTuberia(fdPipeffAstar);
			/*mostrar(&costes);*/
			liberar(&costes);
			/*mostrar(&heuristicas);*/
			liberar(&heuristicas);
	...
	} /*fin del else de arriba del todo*/
...

{
  ...
  printf( "#time spent: %7.2f seconds instantiating %d easy, %d hard action templates", 
...

{
  ...
  while ( --argc && ++argv ) {
	if ( *argv[0] == '.') // Finalizar tratamiento por ser los parametros de path-planning
	  return TRUE;
